...
{
    friend class World;
    static constexpr const u32 INITIAL_RESERVED_NUMBER_OF_CHILDREN = 2;

public:
    Entity(entt::entity entity, entt::registry &entities,
    ...
    template <typename T, typename Callable> void PatchComponent(Callable &&);
...
inline ComponentType &Entity::AddComponent(ComponentType &&component)
{
    ...
    return mEntities.emplace<DecayedComponentType>(
        mEntity, std::forward<ComponentType>(component));
}

template <typename T> inline T const &Entity::GetComponent() const
{
    return mEntities.get<T>(mEntity);
}

template <typename T> inline T *Entity::TryGetComponent() const
{
