...
#define GET_INST_FN_INT(pf, vk, in, inst)                                      \
    {                                                                          \
        in =                                                                   \
            reinterpret_cast<pf>(jnrGetInstanceProcAddr(inst, STRINGIFY(vk))); \
        ThrowIfFailed(in != nullptr, "Unable to get a pointer to " #vk);       \
    }
...
#define GET_INST_FN_INT_OPT(pf, vk, in, inst)                                  \
    {                                                                          \
        in =                                                                   \
            reinterpret_cast<pf>(jnrGetInstanceProcAddr(inst, STRINGIFY(vk))); \
        if (in == nullptr)                                                     \
            SHOWWARNING("Unable to get a pointer to " #vk);                    \
    }
#define GET_INST_FN_OPT(fn, inst)                                              \
    GET_INST_FN_INT_OPT(PFN(fn), vk##fn, jnr##fn, inst)
...
#define GET_DEV_FN_OPT(fn, inst)                                               \
    GET_DEV_FN_INT_OPT(PFN(fn), vk##fn, jnr##fn, inst)
...
{
    jnrGetInstanceProcAddr = reinterpret_cast<PFN_vkGetInstanceProcAddr>(
        glfwGetInstanceProcAddress(NULL, "vkGetInstanceProcAddr"));
    ThrowIfFailed(jnrGetInstanceProcAddr != nullptr,
                  "Unable to get a pointer to vkGetInstanceProcAddr");
...
void Vulkan::LoadFunctionsDevice(VkDevice device)
{
    ...
    GET_DEV_FN(DestroyFramebuffer, device);
...
PFN_vkVoidFunction Vulkan::GetFunctionByName(char const *name, void *userData)
{
    ...
    PFN_vkVoidFunction returnValue =
        jnrGetDeviceProcAddr(deviceInstance->device, name);
    if (returnValue)
    {
        return returnValue;
    }
